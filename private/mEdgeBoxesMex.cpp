...
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] )
{
  ...
  if(nr != 16) mexErrMsgTxt("Sixteen inputs required.");
  ...
  int n = mxGetScalar(pr[14]);
  Boxes boxes(n);
  double *bbs = (double*) mxGetData(pr[15]);
  edgeBoxGen.clusterEdges(E,O,V);
  edgeBoxGen.prepDataStructs(E);

  for(int i=0; i<n; i++) {
    boxes[i].c = bbs[i + 0*n]-1;
    boxes[i].r = bbs[ i + 1*n ]-1;
    boxes[i].w = bbs[ i + 2*n ];
    boxes[i].h = bbs[ i + 3*n ];    
    
    edgeBoxGen.scoreBox(boxes[i]);
  }
  ...
  float *outPtr = (float*) mxGetData(pl[0]);
  for(int i=0; i<n; i++) {
    outPtr[ i + 0*n ] = (float) boxes[i].c+1;
    outPtr[ i + 1*n ] = (float) boxes[i].r+1;
    outPtr[ i + 2*n ] = (float) boxes[i].w;
    outPtr[ i + 3*n ] = (float) boxes[i].h;
    outPtr[ i + 4*n ] = boxes[i].s;
  }
